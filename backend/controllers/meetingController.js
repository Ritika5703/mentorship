...
const acceptMeeting = async (req, res) => {
    try {
        ...
        const meeting = await Meeting.findByIdAndUpdate(id, { status: "accepted" }, { new: true });
        ...
        const notification = new notifications({
            ...
            sender: req.user.id,
            meetingId: id,
            message: `Your meeting with ${req.user.name} has been accepted.`,
        });
        notification.save();
        
        res.json({ success: true, meeting, message: "Meeting accepted successfully" });
    }
    catch (error) {
...
const rejectMeeting = async (req, res) => {
    try {
        ...
        const meeting = await Meeting.findByIdAndUpdate(id, { status: "rejected" }, { new: true });
        ...
        res.json({ success: true, meeting, message: "Meeting rejected successfully" });
    }
    catch (error) {
